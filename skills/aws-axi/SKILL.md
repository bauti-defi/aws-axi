...
- **`--query` bypass**: `--query` is forwarded to the underlying `aws` call; when present, aws-axi returns the raw JMESPath result instead of the curated TOON projection. Output is **unbounded** — botocore auto-pages all results (the default cap is suppressed). To bound output size when using `--query`, pass `--max-items N` (or `--limit N` on `logs`). **Exception — secret redaction takes precedence over `--query`**: on secret-bearing Secrets Manager operations, using `--query` without `--reveal` is a hard error (USAGE_ERROR). Pass `--reveal` to explicitly opt in, then `--query` applies normally.
- **Secrets Manager engine-path redaction**: generic Secrets Manager responses recursively replace `SecretString` and `SecretBinary` values with `<redacted>` by default. Pass `--reveal` to render plaintext; aws-axi consumes that wrapper-only flag and never forwards it to the AWS CLI.
- **Byte-exact secret retrieval (`--raw`)**: `secretsmanager get-secret-value <id> --reveal --raw` writes only the `SecretString` to stdout — no TOON wrapper, no trailing newline — so it is safe for command substitution. `--raw` requires `--reveal`, cannot be combined with `--query`, and rejects `SecretBinary` secrets with a USAGE_ERROR.
- **`s3 presign` is GET-only**: `s3 presign s3://bucket/key [--expires-in <seconds>]` returns a presigned download URL. `--expires-in` defaults to 3600 and must be 1–604800 seconds; the URI must include an object key, and extra positionals are a USAGE_ERROR. There is no presigned-upload (PUT) mode.
...
- **Duplicate owned flags: last-wins**: when the same value-taking flag appears more than once (e.g. `--role-name old --role-name new`, `--secret-id a --secret-id b`, `--expires-in 60 --expires-in 120`), the last value wins — matching real `aws` CLI behaviour. This makes it safe to override a default the overlay may have injected. Every occurrence is still validated, so a malformed one anywhere in the argv is a `USAGE_ERROR` even if a later occurrence would have won. **Exception — repeated boolean toggles resolve on their FIRST occurrence** (`--dryrun`, `--recursive`, `--reveal`): a trailing duplicate cannot undo the fail-safe direction, so `--dryrun --dryrun=false` stays a dry run and `--reveal=false --reveal` stays redacted.
- **Boolean flag vocabulary (S3 cp/rm and ls s3://...):** Several S3 flags (`--dryrun`, `--recursive`, `--quiet`, `--only-show-errors`, `--no-progress`, `--follow-symlinks`) accept an explicit boolean value on `cp`, `rm`, and `s3 ls s3://...`. Recognised values: `true`/`1`/`yes` (enable) or `false`/`0`/`no` (disable). Bare `--flag` also enables. **Unrecognised values hard-error:** `--dryrun=off` is `USAGE_ERROR` — use `--dryrun=false` or `--dryrun=no` to disable. Exception: `--recursive` on bare `s3 ls` (no URI) and `s3 head-object` is always `USAGE_ERROR` regardless of value (including `=false`) — those operations have no recursion concept and real `aws` rejects `--recursive=false` outright.
- **Boolean operation parameters accept an explicit value:** any botocore boolean (`--start-from-head`, `--unmask`, and the rest) accepts `--flag`, `--flag true|1|yes`, `--flag false|0|no`, the `=` forms, and `--no-flag`. The literal is not forwarded. `--flag false` and `--no-flag` set the parameter false; bare `--flag` and `--flag true` set it true. Last occurrence wins. An unrecognised `=value` (for example `--start-from-head=off`) is `USAGE_ERROR` and names the token. Real `aws` rejects explicit values; aws-axi translates them before the CLI call.
- **Standard-input file references**: an explicit `file:///dev/stdin` AWS argument receives stdin from the wrapper, so `printf '{}' | aws-axi secretsmanager put-secret-value --secret-id id --secret-string file:///dev/stdin` completes safely. Ordinary commands do not read or drain stdin.
