...
- **Boolean flag vocabulary (S3 cp/rm/ls):** Several S3 flags (`--dryrun`, `--recursive`, `--quiet`, `--only-show-errors`, `--no-progress`, `--follow-symlinks`) accept an explicit boolean value. Recognised values: `true`/`1`/`yes` (enable) or `false`/`0`/`no` (disable). Bare `--flag` also enables. **Unrecognised values hard-error:** `--dryrun=off` is `USAGE_ERROR` — use `--dryrun=false` or `--dryrun=no` to disable. This prevents the silent no-op where an unrecognised value was treated as truthy (dry-run on, zero bytes transferred, exit 0).
