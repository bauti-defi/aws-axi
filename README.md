...
| `aws` never reads `.env` from cwd                      | `aws-axi` never reads `.env` from cwd                 | Both honor only exported shell env vars and `~/.aws/*`; a repo's `.env` (e.g. `AWS_ENDPOINT_URL=http://localhost:4566` for LocalStack) is ignored |
...
- **No `.env` loading** — aws-axi does not read `.env` from the current directory. Only genuinely-exported
  shell environment variables and `~/.aws/*` config are honored, matching the `aws` CLI exactly.
